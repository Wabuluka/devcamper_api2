...
exports.getBootcampsInRadius = asyncHandler(async (req, res, next)=>{
    ...
    // calculate the radius of the earth in radians
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next)=>{
    ...
    if(!file.mimetype.startsWith('image')){
        return next(
            new ErrorResponse(`Please upload an image file`, 500)
        )
    }
    // check file size
    if(file.size > process.env.MAX_FILE_UPLOAD){
        return next(
            new ErrorResponse(`Please upload an image below ${process.env.MAX_FILE_UPLOAD}`, 500)
        )
    }
